...
def main():
    ...
    parser.add_argument('-p', action = 'store', dest = 'p',type = int, default = 1680)
    ...
    for i,j in enumerate(list_dir):
        ...
        y = pl.append(y,data[-b_pts*num_ts:,3]%(2.0*pl.pi))
        A_arr = pl.append(A_arr,build*coef)
        
    if (paper):
        pl.hexbin(A_arr,y,gridsize=300,bins="log",mincnt=1,edgecolor = 'none') 
        #H, xe, ye = pl.histogram2d(A_arr,y,bins=190) 
        #cmap=pl.cm.YlOrRd_r
    else:
        pl.hexbin(A_arr,y,gridsize=300,cmap=pl.cm.Greys,bins="log",mincnt=1) 
