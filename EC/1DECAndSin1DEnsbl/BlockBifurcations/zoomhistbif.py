...
def main():
    ...
    if (paper):
        pl.hexbin(A_arr,x,gridsize=1800,bins="log",mincnt=2) 
        #cmap=pl.cm.YlOrRd_r
    else:
        pl.hexbin(A_arr,x,gridsize=1800,cmap=pl.cm.Greys,bins="log",mincnt=0) 
    #pl.hexbin(A_arr,x,bins="log") 

    #H,xedges,yedges = pl.histogram2d(x,A_arr,bins=(100,100))
    #print(xedges)
    ...
    pl.axis([.25,.28,2.0,3.0])
    if paper:
        pl.savefig("paper_zoombif_hist.png",dpi=300)
    else:
        pl.savefig("pres_zoombif_hist.png",dpi=300,transparent = True)
    
    if paper:
        os.system("open paper_zoombif_hist.png")
    else:
        os.system("open pres_zoombif_hist.png")
