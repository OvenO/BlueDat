...
def main():
    ...
    y = 1.0+abs(1.0-y)
    if (paper):
        #pl.hexbin(A_arr,y,gridsize=130,bins="log",mincnt=2) 

        H, xe, ye = pl.histogram2d(A_arr,y,bins=400) 
        #cmap=pl.cm.YlOrRd_r
    else:
        pl.hexbin(A_arr,y,gridsize=300,cmap=pl.cm.Greys,bins="log",mincnt=2) 
    #pl.hexbin(A_arr,x,bins="log") 
    
    H = pl.log(H)
    pl.imshow(H.T,interpolation = "nearest",origin="lower")
    pl.colorbar()
