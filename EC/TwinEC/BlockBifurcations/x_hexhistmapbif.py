...
def main():
    ...
    parser.add_argument('-p', action = 'store', dest = 'p',type = int, default = 121)
    ...
    x = x%(2.0*pl.pi)
    if (paper):
        pl.hexbin(A_arr,x,gridsize=10,bins="log",mincnt=2,edgecolor="none") 
        #H, xe, ye = pl.histogram2d(A_arr,y,bins=190) 
        #cmap=pl.cm.YlOrRd_r
    else:
        pl.hexbin(A_arr,y,gridsize=10,cmap=pl.cm.Greys,bins="log",mincnt=2) 
