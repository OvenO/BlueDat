...
def main():
    ...
    first_time = pl.arange(0.0,.1/dt,dt)
    ...
    x_other = x0+pl.array([0.0,epsilon,0.0])
    ...
    for i in range(num + throw_away):
     ...
     # plot of atractor
        ax.plot(sol[:,0],sol[:,1],sol[:,2],color="Red")
    ...
    #NOW CALCULATE LE
    le = 0.0
    for i,j in enumerate(final_dist_arr):
        le+=pl.log(j/epsilon)
    le = le/len(final_dist_arr)
