...
def find_one_full_closed(sol,thresh,dt):
    ...
    while ((loc*dt)%(2*pl.pi)>dt):
        loc-=1
    first_loc = loc 
    print(str(pl.shape(sol)))
    print(first_loc)
    first_pnt = sol[first_loc-1,:]
    loc-=1
...
def main():
    ...
    # this is a period-4 orbit
    totIter = int(16*pl.pi/dt)
    totTime = totIter*dt
    time = pl.arange(0.0,16*pl.pi,dt)
    ...
    while A > A_max:
        ...
        # for this version only 8*pi loop
